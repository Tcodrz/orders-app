...
export const sortByPrice = (orders: IOrder[]): IOrder[] => [...orders].sort((a, b) => a.price - b.price);

export const sortByPriceReverse = (orders: IOrder[]): IOrder[] => [...orders].sort((a, b) => b.price - a.price);
...
export const filter = (orders: IOrder[], filterObj: FilterState): IOrder[] => {
    const filteredOrders = orders
        ...
        .filter(order => (filterObj.fromAmount) ? order.price >= filterObj.fromAmount : order)
        .filter(order => (filterObj.toAmount) ? order.price <= filterObj.toAmount : order)
        .filter(order => {
            let match = false;
            if (filterObj.narratorName.length > 0) {
                order.narrators.forEach(n => {
                    if (n.name === filterObj.narratorName) { match = true; }
                });
            } else { return order; }
